...
backup_generate_model 'gitlab_repos' do
  ...
  store_with({
    'engine' => 'S3',
    'settings' => {
      's3.region'            => node[:gitlab][:backup][:aws][:s3_region],
      's3.bucket'            => node[:gitlab][:backup][:aws][:s3_bucket],
      's3.path'              => node[:gitlab][:backup][:aws][:s3_path] + node.name,
      's3.keep'              => node[:gitlab][:backup][:aws][:s3_keep],
